    ...
    .config(function ($routeProvider) {
        $routeProvider
            .when('/', {
                ...
                controller: 'NBDemoCtrl'
            })
            .when('/bar-demo', {
                templateUrl: 'views/barchart-demo.html',
                controller: 'NBDemoCtrl'
            })
            .when('/hist-demo', {
                templateUrl: 'views/hist-demo.html',
                controller: 'NBDemoCtrl'
            })
            .when('/bar-hist-demo', {
                templateUrl: 'views/bar-hist-demo.html',
                controller: 'NBDemoCtrl'
            })
            .when('/upload-demo', {
                templateUrl: 'views/upload-demo.html',
                controller: 'NBDemoCtrl'
            })
            .when('/css3-demo', {
                templateUrl: 'views/css3-demo.html',
                controller: 'NBDemoCtrl'
            })
            .when('/multivar-demo', {
                templateUrl: 'views/multivar-demo.html',
                controller: 'NBDemoCtrl'
            })
            .when('/spreadsheet-demo', {
                templateUrl: 'views/spreadsheet-demo.html',
                controller: 'NBDemoCtrl'
            })
            .otherwise({
                redirectTo: '/'
...
/* global THREE, document, BaseApp, d3, nv, $, _,  angular, crossfilter */
'use strict';

BaseApp.controller('NBDemoCtrl', ['$scope', '$http', 'baseData', '$upload', '$compile', function ($scope, $http, baseData, $upload, $compile) {
    // default sort value
    $scope.valueType = 'total_order_value';
    $scope.consts = {
        pos: {TOP_LEFT: [-1000, 800, 0], TOP_RIGHT: [1000, 800, 0]}
    };
    $scope.data = {raw: []};

    $scope.demo_pages = [
        {'url': '#/bar-demo', 'title':'Basic barchart demo'},
        {'url': '#/hist-demo', 'title':'Active histograms'},
        {'url': '#/bar-hist-demo', 'title':'Barcharts and histograms'},
        {'url': '#/upload-demo', 'title':'Charts spawned from JSON-databall'},
        {'url': '#/css3-demo', 'title':'A minor report - CSS3 data-viewer'},
        {'url': '#/multivar-demo', 'title':'Multivariate time-lines'},
    ];

    // multivarDemo settings
    $scope.multivarDemo = {};
    // $scope.multivarDemo.dataURL = '/static/data/walking_data/1.csv';
    $scope.multivarDemo.dataURL = '/static/data/demos/iris.csv';
    // $scope.multivarDemo.cols = ['x', 'y', 'z'];
    $scope.multivarDemo.cols = ['sepal length', 'sepal width', 'petal length', 'petal width'];
    $scope.multivarDemo.title = "Edgar Anderson's Iris data set, scatterplot matrix";
    $scope.multivarDemo.colorColumn = 'species';
    $scope.multivarDemo.legend = {};
    $scope.multivarDemo.legend.data = ['setosa', 'versicolor', 'virginica'];

    $scope.multilineDemo = {};
    // $scope.multivarDemo.dataURL = '/static/data/walking_data/1.csv';
    $scope.multilineDemo.dataURL = '/static/data/walking_data/1.csv';
    // $scope.multivarDemo.cols = ['x', 'y', 'z'];

    // file-upload programmatic directives demo
    $scope.onFileSelect = function($files) {
        ...
        for (var i = 0; i < $files.length; i++) {
            ...
            $scope.upload = $upload.upload({
                ...
                fileFormDataName: 'data', //or a list of names for multiple files (html5).
                /* customize how data is added to formData. See #40#issuecomment-28612000 for sample code */
                //formDataAppender: function(formData, key, val){}
            }).progress(function(evt) {
                console.log('percent: ' + parseInt(100.0 * evt.loaded / evt.total));
            }).success(function(dataURL, status, headers, config) {
                // file is uploaded successfully
                console.log(dataURL);
                if(dataURL){
                    baseData.get(dataURL, function(data) {
                        $scope.initData(data);
                        $scope.createDirectives(data);
                    });
                }
            });
            //.error(...)
    ...
    $scope.createDirectives = function(data) {
        var el,
        chartGroup = angular.element(document.createElement('chart-group')),
        keys = Object.keys(data.options || data.data[0]),
        colWidth = parseInt(Math.max(2, 12/keys.length), 10);
        // We need a chart-group
        angular.element('div.ngview').append(chartGroup);
        // Now the navbar-filter..
        $scope.createDirective('navbar-bdk', {'data':'data'});
        // Now the charts...

        keys.forEach(function(k) {
            $scope.createDirective('ssbarchart', {'class':'col-md-' + colWidth, 'data':'data', 'col':k});
        });
    };

    $scope.createDirective = function(type, attrs) {
        var el, dir = angular.element(document.createElement(type));
        _.each(attrs, function(v, k) {
            dir.attr(k, v);
        });
        el = $compile(dir)($scope);
        angular.element('chart-group').append(el);
    };
    ...
    $scope.initData = function(data) {
        $scope.data = {
            ...
            menuData: [],
            orderedData: [],
            gen: {filtered:0, subFiltered: 0, menuData:0, orderedData:0}
        };

        $scope.generateMenuData = function(data) {
            var filter = crossfilter(data);
        };

        $scope.$on('barclicked', function(e) {
            console.log('barclicked: ' + e);
        });
        // we watch the filtered data in navbar for change
        $scope.$watch('data.filtered.length', function(newData, oldData) {
            if(newData !== oldData){ // watches can be called twice - should be idempotent
                console.timeStamp('filtered data changed, new size: ' + newData);
                $scope.data.orderedData = _.sortBy($scope.data.filtered, function(d) {
                    return -d[$scope.valueType];
                });
                // our 'live' subFiltered data is initially set to this
                $scope.data.subFiltered = $scope.data.orderedData;
                console.timeStamp('Data sorted and out...');
            }
        });

        $scope.$watch('data.subFiltered.length', function(newData, oldData) {
            // convenience container
            $scope.data.menuDataByCol = {};
            $scope.data.menuData.forEach(function(d) {
                $scope.data.menuDataByCol[d.name] = d;
            });
...
BaseApp.factory('baseData', ['$http', function($http) {
        'use strict';
        var SERVER_URL = '';
        return {
            get: function(url, callback){
                $http.get(SERVER_URL + url).success(callback);
            },
...
(function(ng){
    ...
    if(!document.URL.match(/\?nobackend$/)){
        return; // not using this stubbed backend
    }

    console.log('======= !NOTE! USING STUBBED BACKEND =======');
...
/* global document, my3D, THREE, d3, angular, window, $, _, clearTimeout, kutils, kcharts, ssnavbar, nv, crossfilter */
'use strict';


angular.module('3DApp', []);
angular.module('BaseApp')
    .directive('dataplane', ['$window', '$timeout', '$rootScope', function($window, $timeout, $rootScope) {
        return{
            restrict: 'EA',
            scope: {
                pos: '@',
            ...
            require: ['^viewer3d', 'dataplane'],
            controller: ['$scope', function($scope) {
                this.pos = typeof $scope.pos !== 'undefined'? JSON.parse($scope.pos): [-1000, 800, 0];
                this.orientation = $scope.orientation == 'vertical'? my3D.VERTICAL: my3D.HORIZONTAL;
                this.padding = typeof $scope.padding !== 'undefined'? parseInt($scope.padding, 10): 10;
                this.dataplane = null;
                this.registerChart = function(chart) {
                    console.log('Chart registered with DataPlane: ' + chart);
                    return $scope.viewer.addChart(chart, this.dataplane);
                    // this.dataplane.addChart(chart);
                };
                this.onTitleClick = function(chart) {
                    this.dataplane.focusChart(chart);
                };

            }],
            link: function(scope, ele, attrs, ctrls){
                var dpctrl = ctrls[1], vctrl = ctrls[0];
                console.log('Creating Data-plane');

                scope.$watch(function() {
                    return vctrl.viewer3d;
                },function(viewer) {
                    if(viewer){
                        scope.viewer = viewer;
                        dpctrl.dataplane = new my3D.DataPlane(viewer, new THREE.Vector3(dpctrl.pos[0], dpctrl.pos[1],  dpctrl.pos[2]), dpctrl.orientation, dpctrl.padding);
                    }
                });
            }
        };
...
angular.module('BaseApp')
    .directive('viewer3d', ['$window', '$timeout', '$rootScope', function ($window, $timeout, $rootScope) {
    return {
        restrict: 'EA',
        scope: {
            // 'width': '=',
        ...
        link: function(scope, ele, attrs, ctrl) {
            var v3d = this;
            scope.height = parseInt(attrs.height, 10) || 500;
            scope.width = parseInt(attrs.width, 10) || d3.select(ele[0]).node().offsetWidth;
            scope.aspectRatio = scope.width/scope.height;
            if($rootScope.debugFlag){
                scope.debugBox = d3.select('.container')
                    .append('div').attr('class', 'debugbox')
                ;
            }
            // the renderer has absolute positioning so need to specify height of div
            if(typeof attrs.fullscreen !== 'undefined'){
                $('.container').width('100%');
                $('.navbar').hide();
                $('footer').hide();
                scope.body = $('.ngview').css('background', '#000').css('padding-top', '0px').css('color', '#eee');
                scope.container = $('.container').css('width', '100%').css('background', '#000');
                scope.width = scope.body.width();
                }
            // ele.css('height', scope.height + 'px');
            // ele.css('width', scope.width + 'px');
            ele.append("<div class='viewer3d'></div>");
            ele.after("<div id='v3dmenu'></div>");
            ctrl.viewer3d = scope.$parent.viewer3d = new my3D.BasicCSS3DViewer().init(ele.find('.viewer3d'), scope.width, scope.height);
            // ctrl.viewer3d = scope.$parent.viewer3d = new my3D.BasicCSS3DViewer().init($('body').find('.viewer3d'), scope.width, scope.height);
            // ctrl.viewer3d = scope.$parent.viewer3d = new my3D.BasicCSS3DViewer().init(scope.container, scope.width, scope.height);
            ctrl.viewer3d.animate();
            ...
            scope.$watch(function() {
                ...
                // to preserve aspect ratios etc demands something clever here. Hmm...
                ctrl.viewer3d.windowResize(scope.body.width(), scope.body.width()/scope.aspectRatio);
                // ctrl.viewer3d.windowResize(ele.width(), ele.width()/scope.aspectRatio);
                scope.render();
            });

            scope.render = function() {
                ctrl.viewer3d.render();
            };

            scope.objects = [];

            scope.resetCamera = function() {
                ctrl.viewer3d.resetCamera();
            };

            scope.toggleTrackball = function() {
                ctrl.viewer3d.toggleTrackball();
            };
            // add menu options
            d3.select('#v3dmenu').selectAll('buttons')
                .data([
                    {title: 'Reset Camera', cbk: scope.resetCamera.bind(v3d)},
                    {title: 'Toggle trackball', cbk: scope.toggleTrackball.bind(v3d)}

                ]).enter()
                .append('button')
                .attr('class', 'btn btn-blue btn-small')
                .html(function(d) {
                    return d.title;
                })
                .on('click', function(d) {
...
angular.module('3DApp')
    .directive('ng-webgl', ['$window', '$timeout', '$rootScope', 'baseData', function ($window, $timeout, $rootScope, baseData) {
    return {
      restrict: 'A',
      scope: {
        'width': '=',
...
/* global my3D, d3, angular, window, $, _, clearTimeout, kutils, kcharts, navbar, nv, crossfilter */
'use strict';

angular.module('BaseApp')
    .directive('chartGroup', ['$window', '$timeout', '$rootScope', 'baseData', function ($window, $timeout, $rootScope, baseData) {
        return {
            restrict: 'EA',
            scope: {
                data: '=',
            ...
            link: function(scope, ele, attrs) {
                ...
                if(attrs.viewer){
                    this.viewer = my3D.BasicCSS3DViewer(ele, 400, 400);
                }
                if(dataURL){
                    baseData.get(dataURL, function(data) {
                        scope.$parent.initData(data);
                    });
                }
            }
        };
...
angular.module('BaseApp')
    .directive('filterMenu', ['$window', '$timeout', '$rootScope', 'd3Service', function ($window, $timeout, $rootScope, d3Service) {
        return {
            restrict: 'EA',
            scope: {
                data: '=',
            ...
            template: "<div id='filter-menu-bar'><ul class='menu-list'></ul></div>",
            link: function(scope, ele, attrs) {
                console.timeStamp('Into navbar link');
                // console.profile('into navbar link');
                var d3 = d3Service,
                renderTimeout;
                ...
                scope.render = function(data) {
                    if(angular.isDefined(data)){
                        ...
                        renderTimeout = $timeout(function() {
                            ...
                            scope.nb = navbar.testmenu = new navbar.FilterMenuAng().init(data.options).setData(data, function() {
                                scope.$apply();
                            });
                            $rootScope.navbar = scope.nb;
                            // type-2 selected by default, for example
                            // scope.nb.setColumnRowCurrent('type', '2', true);
                        }, 0);
                    }
                };

                scope.$on('barclicked', function(e, evbc) {
                    // evbc is the callback data from the chart-click event
                    console.log('in navbar, barclicked: ' + evbc);
                    var filter = scope.nb.getLastFilter();
                    filter.setColumnRow(evbc.series.key, evbc.point.label);
                    filter.updateHandler();
                });

                scope.$on('updateMenuData', function(e, data) {
                    // domain change on a line-chart - trigger menu-change on new data
                    scope.nb.getMenuData(crossfilter(data), false);
                    // we need to set the filtered data for heat-maps etc.
                    scope.data.subFiltered = data;
                    // this is needed to trigger the watches
                    scope.$apply();
                });

                scope.$watch('data.raw', function(data) {
                    if(data.length > 0){
                        scope.render(scope.data);
                    }
                });
                console.timeStamp('Out navbar link');
            }
        };
    }]);


// some shared chart book-keeping, attaching stuff to scope etc.
kcharts.chartLink = function(scope, ele, attrs, ctrl, $window, $timeout) {
    scope.height = parseInt(attrs.height, 10) || parseInt(ele.parent().attr('height')) || 300;
    scope.width = parseInt(attrs.width, 10);
    scope.margin = parseInt(attrs.margin, 10) || 20;
    scope.margins = {top: scope.margin, right: scope.margin, left: scope.margin, bottom: scope.margin};

    ele.addClass('kchart');

    scope.onTitleClick = function() {
        ctrl.onTitleClick(ele[0]);
    };

    scope.title = d3.select(ele[0])
        .append('h4')
        .attr('class', 'chart-title')
        .style('text-align', 'center')
        .text(kutils.titleizeString(attrs.title || scope.getTitle && scope.getTitle() || attrs.col || 'Title to be decided...'))
        .on('click', function() {
            console.log('You clicked me: ' + ele[0]);
            if(scope.onTitleClick){
                scope.onTitleClick();
            }
            })
    ;
    // ctrl is inherited controller from DOM-parent
    if(ctrl){
        $timeout(function() {
            if(ctrl){ // are we inside a dataplane
                scope.object = ctrl.registerChart(ele[0]);
            }
        },0);
        ...
        d3.select(ele[0])
            .on('mousedown', function(e) {
                if(ctrl){
                    ctrl.dataplane.viewer.selectGroup(scope.object.inGroup);
                    console.log('You clicked a bar-chart');
                }
            })
            .on('mouseover', function() {
                if(!this.obj.inFocus){d3.select(this).classed('highlight', true);}
                ctrl.dataplane.viewer.setMouseOverGroup(scope.object.inGroup);
            })
            .on('mouseout', function() {
                d3.select(this).classed('highlight', false);
                ctrl.dataplane.viewer.mouseOverGroup = false;
            });
    }
    ...
    scope.$watch(function() {
        return d3.select(ele[0]).node().offsetWidth;
    }, function(w) {
        if(!scope.data || !scope.data.length){return;}
        // scope.chart.dimensions({width:w});
        scope.render(scope.data);
    });
};


kcharts.dataWatch = function(scope) {
    // Watch for data-change
    var watchFn = function(newData, oldData) {
        if(newData !== oldData){
            if(!newData){return;}
            scope.render(scope.data);
        }
    ...
    scope.$watch('data.subFiltered.length',  watchFn);
    scope.$watch('data.filtered.length', watchFn);
};


// A customized nv-barchart with call-backs and watching the controllers 'data'
angular.module('BaseApp')
    .directive('kbarchart', ['$window', '$timeout', '$rootScope', 'd3Service', function ($window, $timeout, $rootScope, d3Service) {
        return {
            restrict: 'EA',
            require: '^?dataplane', // optional to register with CSS3D
            scope: {
                data: '=data',
                // col: '=col',
                label: '@',
            ...
            link: function(scope, ele, attrs, ctrl) {
                kcharts.chartLink(scope, ele, attrs, ctrl, $window, $timeout);
                kcharts.dataWatch(scope);

                scope.svg = d3Service.select(ele[0])
                    .append("svg")
                ...
                scope.chart.discretebar.dispatch.on('elementClick', function(e){
                    console.log('element: ' + e.value);
                    console.dir(e.point);
                    $rootScope.$broadcast('barclicked', e);
                });

                // expects data of the form [{key: foo, values: [{label:bar, value:10}, ...
                scope.refineData = function(data) {
                    data = data.menuDataByCol;
                    if(!(attrs.col in data)){
                        console.log('There is no column with name ' + attrs.col + '!');
                        // shouldn't break things
                        return [];
                    }
                    return [{key: data[attrs.col].key, values: data[attrs.col].categories}];
                };
                // render the new/changed data to our chart
                scope.render = function(data) {
                    data = scope.refineData(data);
                    if(!data){return;}
...
// a customized nv-pie-chart
angular.module('BaseApp')
    .directive('kpiechart', ['$window', '$timeout', '$rootScope', 'd3Service', function ($window, $timeout, $rootScope, d3Service) {
        return {
            restrict: 'EA',
            require: '^?dataplane', // optional to register with CSS3D
            scope: {
            ...
            link: function(scope, ele, attrs, ctrl) {
                scope.donutFlag = attrs.donut?true:false;
                scope.labelType = attrs.labelType || 'percent';
                kcharts.chartLink(scope, ele, attrs, ctrl, $window, $timeout);
                kcharts.dataWatch(scope);

                scope.svg = d3.select(ele[0])
                    .append("svg")
                    .style('height', scope.height + 'px')
                ;
                ...
                scope.chart = nv.models.pieChart()
                    ...
                    // .donut(scope.donutFlag)
                    // .donutRatio(scope.donutRatio)
                ;

                scope.chart.pie.dispatch.on('elementClick', function(e){
                    console.log('element: ' + e.value);
                    console.dir(e.point);
                    $rootScope.$broadcast('pieclicked', e);
                });
...
// A 2D histogram showing 'heat-map' of chosen value (countKey, e.g. TotalOrderValue) at intersection of two variables/dimensions (xKey, yKey, e.g. pressure + ProductFamily)
angular.module('BaseApp')
    .directive('khist2d', ['$window', '$timeout', '$rootScope', 'd3Service', function ($window, $timeout, $rootScope, d3Service) {
        return {
            restrict: 'EA',
            require: '^?dataplane',
            scope: {
            ...
            link: function(scope, ele, attrs, ctrl) {
                scope.colRange = typeof attrs.colrange !== 'undefined'? attrs.colrange: 'BYR';
                // scope.inhouseScale = typeof attrs.inhousescale !== 'undefined'? (attrs.inhousescale === 'true'): false;

                // construct title from x and y keys if not specified
                console.timeStamp('Into hist2d link');
                var ts = kutils.titleizeString;
                ele.addClass('khistchart');
                scope.activeAxes = (attrs.activeaxes === 'true');
                scope.currentMenu = null;
                if(scope.activeAxes){
                    ele.addClass('activeAxes');
                }
                // attrs.title = typeof attrs.title !== 'undefined'? attrs.title: 'Circle size: ' + ts(attrs.countkey) + '(red=inhouse blue=captive)';
                // attrs.title = typeof attrs.title !== 'undefined'? attrs.title: 'Circle size: ' + ts(attrs.countkey);
                // some general book-keeping
                scope.getTitle = function() {
                    var s = 'Circle size: ' + ts(attrs.countkey);
                    if(attrs.countkeycolor && (attrs.countkey != attrs.countkeycolor)){
                        s += ', Color: ' + ts(attrs.countkeycolor);
                    }
                    return s;
                };
                kcharts.chartLink(scope, ele, attrs, ctrl, $window, $timeout);
                // watching both filtered and sub-filtered values for change
                kcharts.dataWatch(scope);
                // synchronize with other menus
                $rootScope.$on('khistAxisChanged', function(e, keycode, value) {
                    scope.changeAxis(keycode, value);
                });
                ...
                scope.svg = d3Service.select(ele[0])
                    ...
                    .style('height', scope.height + 'px')
                ;
                // this directives chart
                var chart = scope.chart = new kcharts.Hist2D();

                scope.changeAxis = function(keycode, value) {
                    chart[keycode](value);
                    // scope.title.text(ts(chart.countKey()) + ' by ' + ts(chart.xKey()) + ' and ' + ts(chart.yKey()));
                    scope.render(scope.data);
                };

                scope.onAxisClick = function(keycode) {
                    // if current menu exists kill it and exit
                    if(scope.currentMenu){
                        scope.currentMenu.remove();
                        scope.currentMenu = null;
                        return;
                    }
                    // we exclude the other axis from choice
                    var excludeKey = (keycode === 'xKey')?chart.xKey(): chart.yKey();
                    var cols, colKey = scope.chart[keycode]();
                    // cols = _.without(Object.keys(scope.data.menuData), chart.xKey(), chart.yKey());
                    // we exclude current axis-dimension but allow for switching of axes
                    cols = _.without(Object.keys(scope.menuData), excludeKey);
                    cols = cols.map(function(d) {
                        return {'text': d, 'value': d};
                    });
                    scope.currentMenu = kcharts.makeMenuBox(ele[0], cols, function(c, menu) {
                        $rootScope.$broadcast('khistAxisChanged', keycode, c.value);
                        menu.remove();
                        scope.currentMenu = null;
                    });
                };
                ...
                chart.onXAxisClick = function(d) {
                    scope.onAxisClick('xKey');
                };

                chart.onYAxisClick = function(d) {
                    scope.onAxisClick('yKey');
                };

                chart.circleRadius = function(d) {
                    // if the value is negative we assume that the color-chart will disambiguate
                    var x = parseInt(chart.rScale(Math.sqrt(Math.abs(d.value)/3.14)),10);
                    if(isNaN(x)){
                        console.log(d);
                        return 1;
                    }
                ...
                chart.fillCircle = function(d) {
                    if(d.value < 0.0){
                        console.log('A negative value: ' + d.value);
                        console.log(d);
                    }
                    if(!attrs.countkeycolor){// possible negatives
                ...
                chart.getRadiusDomain = function() {
                    // if(!scope.inhouseScale){
                        // we want to normalize on the highest profit or loss
                        chart.absMax = d3.max([Math.abs(chart.bounds[0]), Math.abs(chart.bounds[1])]);
                        return [0, Math.sqrt(chart.absMax/3.14)];
                    // }
                ...
                chart.getDomain = function() {
                    if(scope.colRange === 'BYR'){
                        if(!attrs.countkeycolor){
                            // return [0, 0.5, 1.0];
                            // return [-chart.absMax, 0, chart.absMax];
                            return [chart.bounds[0], 0.5 * (chart.bounds[0] + chart.bounds[1]), chart.bounds[1]];
                        }
                        return [chart.boundsColor[0], 0.5 * (chart.boundsColor[0] + chart.boundsColor[1]), chart.boundsColor[1]];
                        // return [0, 0.5, 1.0];
                    }
                    return [0, 1.0];
                };
                ...
                chart.getRange = function() {
                    switch(scope.colRange){
                    case 'BYR':
                        return [kcharts.COLS.blue, kcharts.COLS.yellow, kcharts.COLS.red];
                    case 'YB':
                        return [kcharts.COLS.yellow, kcharts.COLS.blue];
                    case 'YR':
                        return [kcharts.COLS.yellow, kcharts.COLS.red];
                    }
                };
                ...
                chart
                    ...
                    .xKey(attrs.xkey)
                    .yKey(attrs.ykey)
                    .countKey(attrs.countkey)
                    .countKeyColor(attrs.countkeycolor)
                    // .dimensions({'height':scope.height, 'width':scope.width})
                    .height(scope.height)
                    // .width(scope.width)
                    .margins({'top':30, 'bottom':60, 'left':90})
                    .activeAxes(scope.activeAxes)
                    .colorBarPosition({x:-60})
                ;

                scope.refineData = function(_data) {
                    // console.profile('Refining hist data');
                    console.timeStamp('In refine hist2d data');
                    var data = {},
                    counts = {},
                    xKey = chart.xKey(), yKey = chart.yKey(), countKey = attrs.countkey, countKeyColor = attrs.countkeycolor;
                    // we'd prefer menu-columns by key
                    scope.menuData = _data.menuDataByCol;
                    data.xValues = scope.menuData[xKey].categories.map(function(d) {
                        return d.label;
                    });
                    data.yValues = scope.menuData[yKey].categories.map(function(d) {
                        return d.label;
                    });
                    // create hashed object for counting
                    data.xValues.forEach(function(x) {
                        data.yValues.forEach(function(y) {
                            var cdict = counts[x + '__' + y] = {};
                            cdict[countKey] = 0;
                            cdict[countKeyColor] = 0;
                            // cdict[countKeyCaptive] = 0;
                        });
                    });
                    // construct key-pair count of count-key values. !NOTE! probable need for optimization here -> crossfilter an option
                    _data.subFiltered.forEach(function(d) {
                        counts[d[xKey] + '__' + d[yKey]][countKey] += +d[countKey];
                        counts[d[xKey] + '__' + d[yKey]][countKeyColor] += +d[countKeyColor];
                        // counts[d[xKey] + '__' + d[yKey]][countKeyCaptive] += +d[countKeyCaptive];
                        // DEBUG:
                        if(isNaN(counts[d[xKey] + '__' + d[yKey]][countKey])){
                            console.log(d);
                        }
                    ...
                    _.each(counts, function(v, k) {
                        var xy = k.split('__');
                        data.points.push({'x': xy[0], 'y': xy[1], 'value': v[countKey], 'valueColor':v[countKeyColor]});
                    });

                    // console.profileEnd();
                    console.timeStamp('Out refine hist2d data');
                    return data;
                };

                scope.render = function(_data) {
                    if(!_data.subFiltered.length){return;}
                    var data = scope.refineData(_data);
                    scope.svg.datum(data)
                        .transition().duration(1200)
                        .call(chart);
                };

                console.timeStamp('Out hist2d link');
            }
        };
    }]);

angular.module('BaseApp')
    .directive('klinechartFilter', ['$window', '$timeout', '$rootScope', 'd3Service', function ($window, $timeout, $rootScope, d3Service) {
        return {
            restrict: 'EA',
            scope: {
                data: '=data',
            ...
            link: function(scope, ele, attrs) {
                console.timeStamp('Into linebdk link');
                scope.getTitle = function() {
                    return 'Filter-selector for items ordered by ' + kutils.titleizeString(attrs.value);
                };
                kcharts.chartLink(scope, ele, attrs, false, $window, $timeout);
                scope.$watch('data.orderedData.length', function(newData, oldData) {
                    if(newData !== oldData){
                        if(!newData){return;}
                        scope.render(scope.data.orderedData);
                    }
                }, true);
                ...
                var chart = scope.chart = new kcharts.MultiLineChart();

                // d3 = d3Service;
                scope.valueType = attrs.value || 'total_order_value';
                scope.chart.parseData = function(data) {
                    // console.log('In new data parser');
                    this.data = data;
                    this.color = d3.scale.category10();
                   this.color.domain(d3.keys(this.data[0]).filter(function(key) { return key === scope.valueType; }));
                    this.lineData = this.color.domain().map(function(name) {
                        return {
                            name: name,
                            values: data.map(function(d, i) {
                                return {order: i, value: +d[name]};
                            })
                        };
                    });
                ...
                scope.chart.onRedrawCallback = function() {
                    var chart = scope.chart;
                    $rootScope.$broadcast('updateMenuData', this.data.slice(parseInt(this.x.domain()[0], 10), parseInt(this.x.domain()[1], 10)));
                };

                // exercise in extending chart - second axis in this case
                scope.chart.extra = function() {
                    // assume cumsum of first-line
                    var chart=this, line = this.lineData[0],
                    // need to create cum-sum of this
                    cumsum = 0.0,
                    cumlineValues = line.values.map(function(pointData) {
                        var clone = _.clone(pointData);
                        cumsum += pointData.value;
                        clone.value = cumsum;
                        return clone;
                    });
                    chart.cumLineData = {'title': 'Cumulative total', values: cumlineValues};
                    // now we need a new scale for the cumsum
                    chart.y2 = d3.scale.linear().range([chart.h, 0]);
                    chart.y2.domain(d3.extent(_.pluck(chart.cumLineData.values, 'value')));
                    // now the axis
                    chart.y2AxisScale = d3.scale.linear().range([chart.h, 0])
                        .domain([0, 1]);
                    var formatPercent = d3.format(".0%");
                    chart.y2Axis = d3.svg.axis()
                        .scale(chart.y2AxisScale)
                        .ticks(6).tickSize(0,0).orient("right")
                    ...
                    chart.svg.append("g")
                        ...
                        .call(chart.y2Axis);
                    // then the line
                    chart.cline = d3.svg.line()
                        .interpolate('basis')
                        .x(function(d) {return chart.x(d.order);})
                        .y(function(d) {return chart.y2(d.value);});

                    chart.cumline = chart.svg
                        .append('path')
                        .attr("clip-path", "url(#clip-" + chart.id() + ")")
                        .attr("class", "cumline")
                        .attr("data-legend", "Cum. total")
                        .attr('d', chart.cline(chart.cumLineData.values))
                    ;

                };

                scope.tooltipVariables = [];

                var addInfoLine = function(type, value) {
                    return "<strong>" + kutils.titleizeString(type) +":</strong> <span style='color:red'>" + value + "</span>" + "<strong></br>";
                };

                chart.extraMarkers = function() {
                    ...
                    chart.tip = d3.tip()
                        ...
                        .html(function(d) {
                            var s = '';
                            scope.tooltipVariables.forEach(function(t) {
                                s += addInfoLine(t, d[t]);
                            });
                            return s;
                        });

                    chart.container.call(chart.tip);

                    // chart.activeMarkerTooltip = d3.select("body").append("div")
                    //     .attr("class", "tooltip")
                    //     .style("opacity", 0);

                    chart.y1ActiveMarker = chart.svg.append("circle")
                        .attr("r", 4)
                        ...
                        .style("display","none")
                        .on('click', function() {
                            console.log('you clicked circle...');

                        })
                    ;
                    chart.y2ActiveMarker = chart.svg.append("circle")
                        .attr("r", 4)
                        .style("stroke","orange")
                        .style("fill","orange")
                        .style("opacity",1)
                ...
                chart.onMouseMove = function(mpos) {
                    var xcoord = Math.round(chart.x.invert(mpos[0])),
                    xinfo = chart.xName() + ": " + xcoord,
                    y1coord = Math.round(chart.lineData[0].values[xcoord][chart.yKey()]),
                    y1info = "<span style='color:steelblue'>" + chart.yName() + ": " + y1coord + "</span>";

                    chart.y1ActiveMarker.attr("cx", chart.x(xcoord))
                        .attr("cy", chart.y(y1coord))
                        .style("display",null);

                    // var info = chart.data.IDs[xcoord]+" ("+chart.data.names[xcoord]+")<br>"+xinfo+"<br>"+y1info;

                    var y2coord = Math.round(chart.cumLineData.values[xcoord].value),
                    y2info = "<span style='color:orange'>" + chart.y2Name() + ": "+y2coord+"</span>";
                    // info = info + "<br>" + y2info;
                    chart.y2ActiveMarker.attr("cx", chart.x(xcoord)).attr("cy", chart.y2(y2coord)).style("display",null);
                    var d = chart.data[xcoord];
                    d.total_order_value = parseInt(d.total_order_value, 10);
                    d.cum_total = parseInt((100 - 100 * chart.y2(y2coord)/chart.h), 10) + '%';
                    chart.tip.show(chart.data[xcoord]);
                    return;

                    // chart.activeMarkerTooltip.html(info);
                    // chart.activeMarkerTooltip.style("opacity",0.9);
                    // chart.activeMarkerTooltip.style("left", (d3.event.pageX + 30) + "px");
                    // chart.activeMarkerTooltip.style("top", (d3.event.pageY - 30) + "px");
                    // chart.activeMarkerTooltip.style("display",null);

                };

                chart.onMouseOut = function(pos) {
                    chart.y1ActiveMarker.style("display", 'none');
                    chart.y2ActiveMarker.style("display", 'none');
                    chart.tip.hide();
                };

                scope.chart
                    .title('Items by ' + kutils.titleizeString(scope.valueType))
                    .xName('Item order')
                    .yName(kutils.titleizeString(scope.valueType))
                    .xKey('order')
                    // .dimensions({'height':scope.height, 'width':scope.width})
                    .height(scope.height)
                    // .width(scope.width)
                    .margins({'top':10, 'bottom': 70})
                ;
                ...
                scope.render = function(data) {
                    ...
                    data = scope.refineData(data);
                    scope.tooltipVariables = scope.data.menuData.map(function(d) {
                        return d.name;
                    });
                    // hacky clear from early D3 days...
                    scope.svg.select('g').remove('*');
                    scope.svg.datum(data)
                        .transition().duration(1200)
                        .call(scope.chart);
                };
                console.timeStamp('Out linebdk link');
            }
        };
    }]);
...
angular.module('BaseApp')
    .directive('multivar', ['$window', '$timeout', '$rootScope', function ($window, $timeout, $rootScope) {
        return{
            restrict: 'EA',
            scope:{
                data:'='
            ...
            link: function(scope, ele, attrs, ctrl) {
                ...
                d3.csv(scope.data.dataURL, function(data) {
                    var chart = scope.chart = new kcharts.MultiVarChart();
                    chart
                    // .height(scope.height)
                    .width(scope.width)
                    .columns(scope.data.cols)
                    .colorColumn(scope.data.colorColumn)
                    .legendData(scope.data.legend.data)
                    ;
...
angular.module('BaseApp')
    .directive('multiline', ['$window', '$timeout', '$rootScope', function ($window, $timeout, $rootScope) {
        return{
            restrict: 'EA',
            scope:{
                data:'='
